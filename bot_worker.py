...
С появлением веб-калькулятора и алертов (см. app.py) основной точкой входа
для деплоя стал app.py — он сам запускает poll_forever() из этого файла
фоновым потоком, плюс поднимает веб-страницу и алерты. Этот файл при этом
остался рабочим сам по себе: если веб-калькулятор/алерты не нужны —
Procfile можно вернуть на "worker: python bot_worker.py".
