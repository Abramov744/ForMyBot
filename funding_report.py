#!/usr/bin/env python3
"""
Funding Fee Daily Report — Aster + Bybit → Telegram
...
def build_report(start_ms: int, end_ms: int,
                 ...
                 bybit_records: list | None, bybit_error: str | None) -> str:
    ...
    # Итог по всем биржам
    if bybit_totals or aster_totals:
        combined.append("")
        combined.append("💰 Итого по всем биржам:")
        all_assets: set = set(aster_totals) | set(bybit_totals)
        for asset in sorted(all_assets):
            total = aster_totals.get(asset, 0.0) + bybit_totals.get(asset, 0.0)
